...
public class DashboardController : Controller
{
    ...
    public async Task<IActionResult> Index()
    {
        ...
        var actions = await _context.ActionItems.AsNoTracking().ToListAsync();
        ...
        var model = new DashboardViewModel
        {
            ...
            OpenTickets = tickets.Count(t => t.Status == TicketStatus.New || t.Status == TicketStatus.Open),
            InProgressTickets = tickets.Count(t => t.Status == TicketStatus.InProgress || t.Status == TicketStatus.Pending),
            ResolvedTickets = tickets.Count(t => t.Status == TicketStatus.Resolved),
            ClosedTickets = tickets.Count(t => t.Status == TicketStatus.Closed),
            CriticalTickets = tickets.Count(t => t.Priority == TicketPriority.Critical && t.Status != TicketStatus.Resolved && t.Status != TicketStatus.Closed),
            MyAssignedTickets = tickets.Count(t => t.AssignedToId == currentUser.Id && t.Status != TicketStatus.Closed),
            MyReportedTickets = tickets.Count(t => t.ReportedById == currentUser.Id),
            TotalIncidents = incidents.Count,
            OpenIncidents = incidents.Count(i => i.Status != IncidentStatus.Closed),
            CriticalIncidents = incidents.Count(i => i.Severity == IncidentSeverity.Critical && i.Status != IncidentStatus.Closed),
            OpenActions = actions.Count(a => a.Status == ActionStatus.Open || a.Status == ActionStatus.InProgress),
            OverdueActions = actions.Count(a => a.Status != ActionStatus.Completed && a.Status != ActionStatus.Cancelled && a.DueDate < now),
            CompletedActions = actions.Count(a => a.Status == ActionStatus.Completed),
            MyOpenActions = actions.Count(a => a.AssignedToId == currentUser.Id && a.Status != ActionStatus.Completed && a.Status != ActionStatus.Cancelled),
            ...
            TicketsByDepartment = tickets
                .GroupBy(t => t.ReportedBy?.Department ?? "Unknown")
