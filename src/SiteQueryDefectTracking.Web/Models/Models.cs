...
namespace SiteQueryDefectTracking.Web.Models;
...
public sealed class QuerySummaryDto
{
    ...
    {
        QueryStatus.Pending => "text-bg-warning",
        QueryStatus.InProgress => "text-bg-info",
        _ => "text-bg-success"
    };
}
...
public sealed class QuerySearchPayload
{
    ...
    public string ToQueryString()
    {
        var parts = new List<string>
        {
            $"page={Page}",
            $"pageSize={PageSize}",
            $"sortBy={SortBy ?? "DelayDays"}",
            $"sortDirection={SortDirection ?? "desc"}"
        };
        if (!string.IsNullOrWhiteSpace(IPO)) parts.Add($"ipo={Uri.EscapeDataString(IPO)}");
        ...
        if (IssueTypeId.HasValue) parts.Add($"issueTypeId={IssueTypeId}");
        if (Status.HasValue) parts.Add($"status={Status}");
        if (DateFrom.HasValue) parts.Add($"dateFrom={DateFrom:O}");
        if (DateTo.HasValue) parts.Add($"dateTo={DateTo:O}");
        if (MineOnly.HasValue) parts.Add($"mineOnly={MineOnly.Value.ToString().ToLowerInvariant()}");
